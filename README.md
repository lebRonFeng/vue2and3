...
## 插槽
    ...
    3.使用方式：
        ...
        3.作用域插槽
            1.理解：数据在组件自身，但根据数据生成的结构需要组件的使用者来决定。（games数据在Category组件中，但使用组件所遍历出来的结构由App组件决定）
            2.具体编码：
                父组件中：
                <Category>
...
> 备注：若没有网络请求或者其他业务逻辑，组件中也可以越过actions,即不写```dispactch```,直接编写```commit```
